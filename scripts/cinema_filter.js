...
function filterCinemas() {
	const input = document.getElementById('cinemasFilter').value;
	const cinema_name = document.getElementById('cinema_name');
	const cinema_type = document.getElementById('cinema_type');
	const cinema_address = document.getElementById('cinema_address');
	...
	Object.keys(cinemas).forEach(key => {
		const cinema = cinemas[key];

		if (input === key) {
			cinema_name.innerHTML= cinema.locationName;
			cinema_address.innerHTML = cinema.address;
			if (cinema.IMAX) {
				const img = document.createElement("img");
				img.src = 'assets/cinema-types/imax-black.svg';
				cinema_type.appendChild(img);
			}
			if (cinema.LUMIERE) {
				const img = document.createElement("img");
				img.src = 'assets/cinema-types/lumiere-black.svg';
				cinema_type.appendChild(img);
			}
			if (cinema.PREMIERE) {
				const img = document.createElement("img");
